...
export const createInventoryItem = createAsyncThunk('stores/create', async (inventoryItemData, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
    return await storeService.createInventoryItem(inventoryItemData, token);
  } catch (error) {
    const message =
...
export const updateStoreItem = createAsyncThunk('stores/update', async (storeItem, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
export const getAllStoreItems = createAsyncThunk('stores/getAllInventoryItems', async (_, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
export const getStoreItem = createAsyncThunk('stores/getItem', async (storeId, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
export const getItemCountAll = createAsyncThunk('stores/getStoreCountAll', async (_, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
export const getLowStockItemCount = createAsyncThunk('stores/getLowStockItemCount', async (_, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
export const getLowStockItems = createAsyncThunk('stores/getLowStockItems', async (_, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
export const getNoStockItems = createAsyncThunk('stores/getNoStockItems', async (_, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
export const getStoreItemsByCategory = createAsyncThunk('stores/getItemsByCategory', async (category, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
    return await storeService.getInventoryItemsByCategory(category, token);
  } catch (error) {
    const message =
      (error.response && error.response.data && error.response.data.message) || error.message || error.toString();
    return thunkAPI.rejectWithValue(message);
...
export const getItemCategoryCount = createAsyncThunk('stores/getItemCategoryCount', async (_, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
...
export const deleteInventoryItem = createAsyncThunk('stores/delete', async (inventoryItemId, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
    return await storeService.deleteInventoryItem(inventoryItemId, token);
  } catch (error) {
    const message =
      (error.response && error.response.data && error.response.data.message) || error.message || error.toString();
    return thunkAPI.rejectWithValue(message);
...
export const addPurchaseRequest = createAsyncThunk('stores/purchaseRequest', async (purchaseRequest, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
    return await storeService.createPurchaseRequest(purchaseRequest, token);
  } catch (error) {
    const message =
      (error.response && error.response.data && error.response.data.message) || error.message || error.toString();
    return thunkAPI.rejectWithValue(message);
...
export const storesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getAllInventoryItemCategories.fulfilled, (state, action) => {
        state.isLoading = false;
      ...
      .addCase(getAllInventoryItemCategories.rejected, (state, action) => {
        state.isLoading = false;
