...
export const createPurchasing = createAsyncThunk(
  "purchasings/create",
...
export const UpdaterequestStatus = createAsyncThunk(
  ...
  async (purchasingData, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token;
      return await purchasingService.updatePurchasing(purchasingData, token);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const getPurchasing = createAsyncThunk(
  "purchasings/get",
  async (purchasingId, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token;
...
export const GetrequestsCountOdered = createAsyncThunk(
  "purchasings/GetrequestsCountOdered",
  async (_, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token;
...
export const GetrequestsCountPending = createAsyncThunk(
  "purchasings/GetrequestsCountPending",
  async (_, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token;
