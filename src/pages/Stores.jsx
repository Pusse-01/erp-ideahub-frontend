...
function Stores() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      toast.error('Failed to get statistics! Refresh to try again.');
    }

    if (getLowStockItemCountIsError) {
      console.error('Failed to get low stock item count');
    }

    if (getItemCategoryCountIsError) {
      console.error('Failed to get item category count');
    }

    if (getAllInventoryItemCategoriesIsError) {
      console.error('Failed to get inventory item categories!');
    }
    ...
    <>
      <div className="drawer-content-custom f9">
        <div className="grid grid-cols-3 gap-7 w-[92%] mt-7 ">
          <HeadCard icon={folderIcon} value={itemCategoryCount} heading={'Categories'} />
          <HeadCard icon={infoIcon} value={allStoreItemCount} heading={'Total Items'} />
          <HeadCard icon={refreshIcon} value={lowStockItemCount} heading={'Low Stocks'} />
        </div>
        ...
        <div className=" grid grid-cols-12 grid-flow-row gap-1 lg:gap-2 bg-white w-[92%] p-1 lg:p-5">
          <div className="col-start-1 col-span-2 dropdown dropdown-bottom dropdown-end m-1">
            ...
            <ul tabIndex={0} className="dropdown-content z-[1] menu  shadow bg-base-100 rounded-box w-40 ">
              ...
              <li>
                <a id="no-stock" onClick={selectedFilter}>
                  No Stock
                </a>
              </li>
              <li>
                <a id="low-stock" onClick={selectedFilter}>
                  Low stock
                </a>
              </li>
              {storeItemCategories.length > 0
                ? storeItemCategories.map(({ category }) => (
                    <li id={category}>
                      <a id={`${category}`} onClick={selectedFilter}>
                        {category}
                      </a>
                    </li>
                  ))
