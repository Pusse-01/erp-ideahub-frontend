      ...
      <div className="w-[92%] bg-white mb-9">
        ...
        <div className="lg:grid grid-cols-5 gap-2 bg-white p-5 shadow-lg m-2 lg:m-10">
          ...
          <div className={`col-span-1 ${sitemeasurements===null ? 'hidden' : ''}`}>
            <label className="cursor-pointer label text-xs flex items-center justify-center ">
              Site Measurements
            </label>
        ...
        <div className="space-y-2">
          ...
          {enquiryDesigns && enquiryDesigns.length > 0 && enquiryDesigns.map((design, index)=>(
            <div key={design.id} className="collapse collapse-arrow rounded-none shadow-md ">
              ...
              <div className="collapse-title text-sm font-medium grid grid-cols-3  h-11">
                ...
                <div className="col-span-1">
                  Last Updated {design && design.date
                    ?  
                    format(new Date(design.date), 'dd MMMM yyyy')
                    : ""}
                </div>
              ...
              <div className="collapse-content">
                <hr />
                {
                    design && design.files && design.files.map((file, index)=>(
                        <>
                        <div className="p-2">
                        <div  key={index} className="grid grid-cols-6">
                        <p className="col-span-2">{file.itemname}</p>
                        <p className="col-span-2">{file.comment}</p>
                        <div className="col-span-1">{file.outsourced ? <p className="bg-[#C94E4E85] w-fit p-1 rounded text-xs ">Out Sourced</p>:''}</div>
                        <div className="col-span-1 flex items-center justify-center" >
                            <button className="w-5" onClick={() => downloadPdf(file.containername, file.blobname)}><img src={downloadIcon} /></button></div>
                        </div>
                        </div>
